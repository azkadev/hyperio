...
///     .listen((HyperioAzkadevHttpRequestBody body) {
///       ...
///     });
/// ```
///
...
/// ```
class HttpBodyHandler extends StreamTransformerBase<HyperioAzkadevHttpRequest,
    HyperioAzkadevHttpRequestBody> {
  final Encoding _defaultEncoding;

  /// Create a new [HttpBodyHandler] to be used with a [Stream]<[HyperioAzkadevHttpRequest]>,
  /// e.g. a [HttpServer].
  ///
  ...
  ///
  /// The returned [HyperioAzkadevHttpRequestBody] contains a `response` field for accessing
  /// the [HyperioAzkadevHttpResponse].
  ///
  ...
  @override
  Stream<HyperioAzkadevHttpRequestBody> bind(
      Stream<HyperioAzkadevHttpRequest> stream) {
    var pending = 0;
    var closed = false;
...
/// A HTTP content body produced by [HttpBodyHandler] for either [HyperioAzkadevHttpRequest]
...
class HyperioAzkadevHttpRequestBody extends HttpBody {
  /// The wrapped request.
  ///
  /// Note that the [HyperioAzkadevHttpRequest] is already drained, so the
  /// `Stream` methods cannot be used.
  final HyperioAzkadevHttpRequest request;

  HyperioAzkadevHttpRequestBody._(this.request, HttpBody body)
      : super._(body.type, body.body);
}
